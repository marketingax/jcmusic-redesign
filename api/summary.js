...
module.exports = async (req, res) => {
  ...
  const [contacts, events, invoices, opps, txns] = await Promise.all([
    ...
    ghlFetch(`/payments/transactions?altId=${loc}&altType=location&limit=100`),
  ]);
  ...
  const txList = txns.ok ? txns.data.data || [] : [];
  const succeeded = txList.filter((t) => t.status === 'succeeded');
  const revenue = succeeded.reduce((s, t) => s + (Number(t.amount) || 0), 0);
